function SwarmAuth_ELECTION_FIXED
% SWARMAUTH_LIVEDEMO
...
% Red   = Attack / Rejected
    ...
    uilabel(right,'Position',[25 455 520 28],'Text','3. WINGMAN P2P COMMUNICATION', ...
        'FontColor',[0.30 0.95 0.55],'FontWeight','bold','FontSize',15);

    uibutton(right,'push','Text','Wingman ↔ Wingman P2P', ...
        'Position',[25 410 250 40],'ButtonPushedFcn',@(~,~)p2pChat());
    uibutton(right,'push','Text','Send AES-GCM Telemetry', ...
        'Position',[285 410 250 40],'ButtonPushedFcn',@(~,~)telemetry());
    uibutton(right,'push','Text','RUN FULL LIVE DEMO', ...
        'Position',[25 365 510 40], ...
        'BackgroundColor',[0.10 0.55 0.80], ...
        'FontWeight','bold','ButtonPushedFcn',@(~,~)fullDemo());

    uilabel(right,'Position',[25 325 520 28],'Text','PROTOCOL / SECURITY LOG', ...
        'FontColor',[0.75 0.82 0.92],'FontWeight','bold','FontSize',15);

    logBox = uitextarea(right,'Position',[25 65 510 250], ...
    ...
    function runElection()
        % Fault-tolerant Lowest-ID election:
        % NODE_10 (original CH) fails, then NODE_01 also fails.
        % Therefore the lowest SURVIVING ID is NODE_02.
        % Every press represents the CURRENT leader failing.
        % Then the lowest-ID ACTIVE node becomes the new leader.
        if currentLeaderId < 1 || currentLeaderId > N || ~activeNode(currentLeaderId)
            addLog('[ELECTION] Current leader is already offline. No additional leader failure simulated.');
            active_nodes = find(activeNode);
            if isempty(active_nodes)
                setStatus('NO ACTIVE NODES — SWARM OFFLINE');
                return;
            end
        else
            failedLeader = currentLeaderId;
            activeNode(failedLeader) = false;
            authenticated(failedLeader) = false;
            setNode(failedLeader,'red');

            if failedLeader == 10
                leaderLabelH.String = 'NODE_10\nOFFLINE';
            elseif failedLeader >= 1 && failedLeader <= N
                wingLabelH(failedLeader).Visible = 'on';
                wingLabelH(failedLeader).String = sprintf('W%d\nOFFLINE',failedLeader);
            ...
            addLog('==============================================');
            addLog(sprintf('[ALERT] CURRENT LEADER NODE_%02d has failed!',failedLeader));
            setStatus(sprintf('NODE_%02d OFFLINE — LEADER FAILURE DETECTED',failedLeader));
            step();

            addLog('[NETWORK] Wingmen pinging current Cluster Head...');
            setStatus('PINGING LEADER — WAITING FOR RESPONSE');
            for k = find(activeNode)
                setNode(k,'yellow');
                step();
            end
        ...
        active_nodes = find(activeNode);
        if isempty(active_nodes)
            addLog('[CRITICAL] No surviving drones. Swarm is offline.');
            setStatus('SWARM OFFLINE — NO SURVIVING DRONES');
            return;
        end
        ...
        % The winner is ALWAYS the lowest ID among currently active drones.
        newLeader = min(active_nodes);

        addLog(sprintf('[ELECTION] Surviving IDs: %s',sprintf('%02d ',active_nodes)));
        addLog(sprintf('[ELECTION] Lowest SURVIVING ID = NODE_%02d',newLeader));
        addLog(sprintf('[ELECTION] NODE_%02d promotes itself as new Cluster Head.',newLeader));
        setStatus(sprintf('NODE_%02d SELECTED — LOWEST SURVIVING ID',newLeader));
        step();
        ...
        step();
        addLog(sprintf('[SUCCESS] Swarm reorganized around New Cluster Head: NODE_%02d',newLeader));
        addLog(sprintf('[SUCCESS] Press ELECTION again to simulate NODE_%02d failing.',newLeader));
        addLog('==============================================');
    ...
    function authOne(i)
        if ~activeNode(i)
            addLog(sprintf('[REJECT] %s is OFFLINE and cannot authenticate.',ids{i}));
            setStatus(sprintf('%s OFFLINE — authentication skipped',ids{i}));
            return;
        end
        if authenticated(i)
    ...
    function p2pChat()
        % Demonstrates the documented post-authentication UAV-to-UAV P2P mode.
        peers = find(activeNode & (1:N) ~= currentLeaderId);
        if numel(peers) < 2
            addLog('[P2P] Not enough active Wingmen for direct communication.');
            return;
        end
        a = peers(1); b = peers(2);

        addLog('');
        addLog('=== WINGMAN ↔ WINGMAN P2P TELEMETRY ===');
        addLog('[P2P] Authentication gate already passed.');
        if ~authenticated(a), authOne(a); end
        if ~authenticated(b), authOne(b); end

        if ~(authenticated(a) && authenticated(b))
            addLog('[P2P] Communication blocked — peers are not authenticated.');
            return;
        end

        addLog(sprintf('[P2P] WINGMAN_%02d → WINGMAN_%02d',a,b));
        addLog('[P2P] Cluster Head is BYPASSED for operational telemetry.');
        addLog('[P2P] Swarm Group Key used for protected peer traffic.');

        % Draw a temporary direct peer-to-peer communication path.
        h = plot(ax,[wingPos(a,1) wingPos(b,1)], ...
            [wingPos(a,2) wingPos(b,2)], ...
            '-','Color',[0.98 0.70 0.18],'LineWidth',4);
        addLog('[P2P] Direct UAV-to-UAV link established ✓');
        setStatus(sprintf('P2P: WINGMAN_%02d ↔ WINGMAN_%02d ✓',a,b));
        step(); step();
        ...
        addLog('[P2P] Telemetry delivered without routing through the CH ✓');
        setStatus('P2P TELEMETRY DELIVERED ✓');
    end

    function telemetry()
        peers = find(activeNode & (1:N) ~= currentLeaderId);
        if numel(peers) < 2
            addLog('[ERROR] Need two active authenticated Wingmen for telemetry.');
            return;
        end
        sender = peers(1); receiver = peers(2);
        addLog('');
        ...
        message = uint8(sprintf('Telemetry from WINGMAN_%02d: Target locked at Sector 7G.',sender));
        aad = uint8(sprintf('SwarmAuth-v1|W%d|W%d',sender,receiver));
    ...
    function updateAuthCounter()
        if isvalid(authCounter)
            if currentLeaderId <= N && activeNode(currentLeaderId)
                total = sum(activeNode) - 1;
            else
                total = sum(activeNode);
            end
            authCounter.Text = sprintf('AUTHENTICATED: %d / %d',sum(authenticated),total);
            if total > 0 && sum(authenticated) == total
    ...
    function rebuildEdges()
        ...
        for k = 1:N
            ...
            if activeNode(k)
                ...
                ex(q:q+2) = [NaN; NaN; NaN];
                ey(q:q+2) = [NaN; NaN; NaN];
            end
        end
        edgeH.XData = ex;
        edgeH.YData = ey;
